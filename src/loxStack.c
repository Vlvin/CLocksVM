...
void LoxStack_init(LoxStack *self) { LoxStack_reset(self); }

void LoxStack_reset(LoxStack *self) { *self = (LoxStack){0}; }

void LoxStack_free(LoxStack *self) { LoxStack_reset(self); }
...
void LoxStack_print(LoxStack *self) {
  printf("    ");
  for (LoxValue *slot = self->data; slot < self->topElement; slot++) {
    printf("[ ");
    printValue(*slot);
    printf(" ]");
  }
