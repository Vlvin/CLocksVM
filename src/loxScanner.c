...
bool isDigit(const char ch) { return ('0' <= ch && '9' >= ch); }
bool isAlphaNumeric(const char ch) { return (isAlpha(ch) || isDigit(ch)); }

void LoxScanner_init(LoxScanner *self, const char *source) {
  (*self) = (LoxScanner){source, source, 1};
}
...
LoxToken LoxScanner_scanToken(LoxScanner *self) {
  ...
  case '!':
    return LoxScanner_makeToken(
        self, LoxScanner_match(self, '=') ? TOKEN_BANG_EQUAL : TOKEN_BANG);
  case '=':
    return LoxScanner_makeToken(
        self, LoxScanner_match(self, '=') ? TOKEN_EQUAL_EQUAL : TOKEN_EQUAL);
  case '<':
    return LoxScanner_makeToken(
        self, LoxScanner_match(self, '=') ? TOKEN_LESS_EQUAL : TOKEN_LESS);
  case '>':
    return LoxScanner_makeToken(self, LoxScanner_match(self, '=')
                                          ? TOKEN_GREATER_EQUAL
                                          : TOKEN_GREATER);
  case '/':
...
bool LoxScanner_isAtEnd(LoxScanner *self) { return (*(self->current) == '\0'); }

char LoxScanner_advance(LoxScanner *self) { return (*(self->current++)); }

char LoxScanner_peek(LoxScanner *self) { return (*(self->current)); }
...
LoxToken LoxScanner_makeToken(LoxScanner *self, TokenType type) {
  return LoxToken_create(type, self->line, self->start,
                         (size_t)(self->current - self->start));
}

LoxToken LoxScanner_errorToken(LoxScanner *self, const char *message) {
  return LoxToken_create(TOKEN_ERROR, self->line, message, strlen(message));
}

TokenType checkKeyword(LoxScanner *self, int offset, const char *rest,
                       TokenType type) {
  return (memcmp(rest, self->start + offset, strlen(rest)) == 0)
             ? type
             : TOKEN_IDENTIFIER;
}
...
TokenType LoxScanner_identifierType(LoxScanner *self) {
  switch (self->start[0]) {
  case 'a':
    return checkKeyword(self, 1, "nd", TOKEN_AND);
  case 'c':
    return checkKeyword(self, 1, "lass", TOKEN_CLASS);
  case 'e':
    return checkKeyword(self, 1, "lse", TOKEN_ELSE);
  case 'i':
    return checkKeyword(self, 1, "f", TOKEN_IF);
  case 'n':
    return checkKeyword(self, 1, "il", TOKEN_NIL);
  case 'o':
    return checkKeyword(self, 1, "r", TOKEN_OR);
  case 'p':
    return checkKeyword(self, 1, "rint", TOKEN_PRINT);
  case 'r':
    return checkKeyword(self, 1, "eturn", TOKEN_RETURN);
  case 's':
    return checkKeyword(self, 1, "uper", TOKEN_SUPER);
  case 'v':
    return checkKeyword(self, 1, "ar", TOKEN_VAR);
  case 'w':
    return checkKeyword(self, 1, "hile", TOKEN_WHILE);
  case 'f':
    if (self->current - self->start > 1)
      switch (self->start[1]) {
      case 'a':
        return checkKeyword(self, 2, "lse", TOKEN_FALSE);
      case 'o':
        return checkKeyword(self, 2, "r", TOKEN_FOR);
      case 'u':
        return checkKeyword(self, 2, "n", TOKEN_FUN);
      }
    break;
  case 't':
    if (self->current - self->start > 1)
      switch (self->start[1]) {
      case 'r':
        return checkKeyword(self, 2, "ue", TOKEN_TRUE);
      case 'h':
        return checkKeyword(self, 2, "is", TOKEN_THIS);
      }
    break;
  }
